...
class SwinBlockSequence(BaseModule):
    ...
    def forward(self, x, skip_x, hw_shape):
        ...
        if self.is_upsample:
            ...
            x = x.permute(0, 3, 1, 2)
            x = self.deconv(x)
            x = x.permute(0, 2, 3, 1).view(B, up_hw_shape[0] * up_hw_shape[1], C // 2)
            return x
        else:
            ...
            x = self.conv(x)
            x = x.permute(0, 2, 3, 1).view(B, hw_shape[0] * hw_shape[1], C * 2)
            return x


@HEADS.register_module()
class UperFormerHead_Deconv(BaseDecodeHead):
